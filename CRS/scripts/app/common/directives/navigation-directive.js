...
.directive("navigationDir", ['$compile', 'AppSettings', function ($compile, appSettings) {
    return {
        restrict: 'EA',
        scope: {
            currentState: '@',
        ...
        link: function ($scope, $element, $attrs) {
            ...
            $scope.$watch('[userName, userAccessLevel]', function () {
    ...
    function createLink(item) {
        ...
        ? String.format(_tags.li, item.state, item.name)
        : String.format(_tags.liOnClick, item.state, item.name, item.onClick);
    }

    // create dropdown menu 
    function createDropdown(menuItem) {
        var html = String.format(_tags.dropdownBegin, menuItem.state, menuItem.name);
        ...
        for (var i = 0; i < subMenu.length; i++) {
            ...
            html += (item.state == '-') ? _tags.liSeparator : String.format(_tags.li, item.state, item.name);
        }
