...
// CREATE new admin.product module (WITH [] injection, this is a CREATION of module, otherwise it's a retrieval)
...
.factory('productFactory', ['$resource', 
    function ($resource) {
        return $resource(__env.apiBaseUrl + 'products/:id', {}, {
...
// controllers
// ***********
.controller("productCtrl", ['$scope', 'productFactory', 
    function ($scope, productFactory) {
        ...
        $scope.save = function () {
            if ($scope.currentItem.productId == 0) {
                productFactory.create($scope.currentItem
                    , function () { $scope.getAllItems(); }     // call back function to refresh page
                    );
            }
            else {
                productFactory.update({ id: $scope.currentItem.productId }, $scope.currentItem
                    , function () { $scope.getAllItems(); }      // call back function to refresh page
                    );
            }
            $scope.listView = true;
        ...
        $scope.delete = function (index) {
            ...
            if (confirm("Delete " + $scope.currentItem.name + ". Are you sure?" + index)) {
                productFactory.delete({ id: $scope.currentItem.productId }
                    , function () { $scope.getAllItems(); }     // call back function to refresh page
                    );
            }
        };
        ...
        /*
        $scope.mockproducts = [
            { "productId": 1, "name": "sample string 1", "price": 1.0 },
            { "productId": 1, "name": "sample string 2", "price": 2.0 }
        ];
        */
